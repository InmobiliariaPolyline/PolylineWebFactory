...
const FEEDS_BY_CAT = {
  ...
  business: [
    ...
    "https://www.construible.es/feed",           // sector construcción
    "https://www.construirenciudad.com/feed"     // sector construcción
  ]
};
...
const getOgImage = async (url) => {
  try {
    const html = await fetch(url, {
      ...
      // algunos sitios bloquean fetch sin UA / CORS; si falla, devolvemos null
    }).then(r => r.text());
...
const normalizeItem = async (item, sourceName) => {
  ...
  // imagen por RSS / media / enclosure / fallback og:image
  let image =
  ...
  return {
    title,
    url: link,
    urlToImage: image || null,      // <-- mismo nombre que usas en Angular
    publishedAt,
...
export async function handler(event) {
  try {
    ...
    clean.sort((a, b) => new Date(b.publishedAt || 0).getTime() - new Date(a.publishedAt || 0).getTime());
